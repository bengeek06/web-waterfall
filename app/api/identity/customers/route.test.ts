 ...
 */
/**
 * Test suite for `/api/identity/customers` handlers (GET and POST).
 */
...
describe("GET /api/identity/customers", () => {
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      ...
      url: "http://localhost:3000/api/identity/customers",
      headers: {
  ...
  describe("MOCK_API=true", () => {
    ...
    it("returns mock customers list", async () => {
      const req = buildReq();
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies request and returns customers", async () => {
        const req = buildReq();
        
        const mockJson = [{ id: "company-001", name: "Test Customer" }];
        const mockRes = {
            status: 200,
            headers: {
        ...
        expect(global.fetch).toHaveBeenCalledWith(
            `${IDENTITY_SERVICE_URL}/customers`,
...
describe("POST /api/identity/customers", () => {
  const IDENTITY_SERVICE_URL = "http://identity_service:5002";
  let mockFetch: jest.Mock;
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      text: jest.fn().mockResolvedValue(JSON.stringify({ name: "New Customer" })),
      json: jest.fn().mockResolvedValue({ name: "New Customer" }),
      url: "http://localhost:3000/api/identity/customers",
      headers: {
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies POST request and creates company", async () => {
        ...
        const mockJson = { id: "company-new", name: "New Customer" };
        const mockRes = {
            status: 201,
            headers: {
                get: (key: string) => (key === "content-type" ? "application/json" : null),
