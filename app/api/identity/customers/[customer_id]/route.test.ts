 ...
 */
/**
 * Test suite for `/api/identity/customers/cust-001` handlers.
 */
...
describe("/api/identity/customers/cust-001", () => {
  const IDENTITY_SERVICE_URL = "http://identity_service:5002";
  const CUSTOMER_ID = "customer-id-001";
  let mockFetch: jest.Mock;
  let GETFn: (req: NextRequest, context: { params: Promise<{ customer_id: string }> }) => Promise<Response>;
  let PUTFn: (req: NextRequest, context: { params: Promise<{ customer_id: string }> }) => Promise<Response>;
  let PATCHFn: (req: NextRequest, context: { params: Promise<{ customer_id: string }> }) => Promise<Response>;
  let DELETEFn: (req: NextRequest, context: { params: Promise<{ customer_id: string }> }) => Promise<Response>;

  const buildReq = (body?: string): Partial<NextRequest> => {
    return {
      text: jest.fn().mockResolvedValue(body || ""),
      json: jest.fn().mockResolvedValue(body ? JSON.parse(body) : {}),
      url: `http://localhost:3000/api/identity/customers/cust-001`,
      headers: {
  ...
  const buildContext = () => ({
    params: Promise.resolve({ customer_id: CUSTOMER_ID })
  });
  ...
  describe("GET - MOCK_API=true", () => {
    ...
    it("returns mock Customer by ID", async () => {
      const req = buildReq();
  ...
  describe("GET - MOCK_API=false", () => {
    ...
    it("proxies GET request", async () => {
        ...
        const mockJson = { id: "customer-id-001", name: "Test Customer" };
        const mockRes = {
            status: 200,
            headers: {
  ...
  describe("PUT - MOCK_API=false", () => {
    ...
    it("proxies PUT request", async () => {
        ...
        const mockJson = { id: "customer-id-001", name: "Updated" };
        const mockRes = {
            status: 200,
            headers: {
                get: (key: string) => (key === "content-type" ? "application/json" : null),
  ...
  describe("PATCH - MOCK_API=false", () => {
    ...
    it("proxies PATCH request", async () => {
        ...
        const mockJson = { id: "customer-id-001", name: "Patched" };
        const mockRes = {
            status: 200,
            headers: {
                get: (key: string) => (key === "content-type" ? "application/json" : null),
