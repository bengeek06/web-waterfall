 ...
 */
/**
 * Test suite for `/api/identity/init-db` handlers (GET and POST).
 */
...
describe("GET /api/identity/init-db", () => {
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      ...
      url: "http://localhost:3000/api/identity/init-db",
      headers: {
  ...
  describe("MOCK_API=true", () => {
    ...
    it("returns mock initialization status", async () => {
      const req = buildReq();
...
describe("POST /api/identity/init-db", () => {
  const IDENTITY_SERVICE_URL = "http://identity_service:5002";
  let mockFetch: jest.Mock;
