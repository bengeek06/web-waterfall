 ...
 */
/**
 * Test suite for `/api/identity/companies` handlers (GET and POST).
 */
...
describe("GET /api/identity/companies", () => {
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      ...
      url: "http://localhost:3000/api/identity/companies",
      headers: {
  ...
  describe("MOCK_API=true", () => {
    ...
    it("returns mock companies list", async () => {
      const req = buildReq();
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies request and returns companies", async () => {
        const req = buildReq();
        
        const mockJson = [{ id: "company-001", name: "Test Company" }];
        const mockRes = {
            status: 200,
            headers: {
        ...
        expect(global.fetch).toHaveBeenCalledWith(
            `${IDENTITY_SERVICE_URL}/companies`,
...
describe("POST /api/identity/companies", () => {
  const IDENTITY_SERVICE_URL = "http://identity_service:5002";
  let mockFetch: jest.Mock;
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      text: jest.fn().mockResolvedValue(JSON.stringify({ name: "New Company" })),
      json: jest.fn().mockResolvedValue({ name: "New Company" }),
      url: "http://localhost:3000/api/identity/companies",
      headers: {
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies POST request and creates company", async () => {
        ...
        const mockJson = { id: "company-new", name: "New Company" };
        const mockRes = {
            status: 201,
            headers: {
                get: (key: string) => (key === "content-type" ? "application/json" : null),
