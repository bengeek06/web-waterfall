 ...
 */
/**
 * Test suite for `/api/identity/users` handlers (GET and POST).
 */
...
describe("GET /api/identity/users", () => {
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      ...
      url: "http://localhost:3000/api/identity/users",
      headers: {
  ...
  describe("MOCK_API=true", () => {
    ...
    it("returns mock users list", async () => {
      const req = buildReq();
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies request and returns users", async () => {
        const req = buildReq();
        
        const mockJson = [{ id: "company-001", name: "Test User" }];
        const mockRes = {
            status: 200,
            headers: {
...
describe("POST /api/identity/users", () => {
  const IDENTITY_SERVICE_URL = "http://identity_service:5002";
  let mockFetch: jest.Mock;
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      text: jest.fn().mockResolvedValue(JSON.stringify({ name: "New User" })),
      json: jest.fn().mockResolvedValue({ name: "New User" }),
      url: "http://localhost:3000/api/identity/users",
      headers: {
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies POST request and creates company", async () => {
        ...
        const mockJson = { id: "company-new", name: "New User" };
        const mockRes = {
            status: 201,
            headers: {
                get: (key: string) => (key === "content-type" ? "application/json" : null),
