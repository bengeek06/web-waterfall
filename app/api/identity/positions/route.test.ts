 ...
 */
/**
 * Test suite for `/api/identity/positions` handlers (GET and POST).
 */
...
describe("GET /api/identity/positions", () => {
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      ...
      url: "http://localhost:3000/api/identity/positions",
      headers: {
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies request and returns positions", async () => {
        ...
        const mockJson = [{ id: "company-001", name: "Test Position" }];
        const mockRes = {
            status: 200,
            headers: {
        ...
        expect(global.fetch).toHaveBeenCalledWith(
            `${IDENTITY_SERVICE_URL}/positions`,
...
describe("POST /api/identity/positions", () => {
  const IDENTITY_SERVICE_URL = "http://identity_service:5002";
  let mockFetch: jest.Mock;
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      text: jest.fn().mockResolvedValue(JSON.stringify({ name: "New Position" })),
      json: jest.fn().mockResolvedValue({ name: "New Position" }),
      url: "http://localhost:3000/api/identity/positions",
      headers: {
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies POST request and creates company", async () => {
        ...
        const mockJson = { id: "company-new", name: "New Position" };
        const mockRes = {
            status: 201,
            headers: {
                get: (key: string) => (key === "content-type" ? "application/json" : null),
