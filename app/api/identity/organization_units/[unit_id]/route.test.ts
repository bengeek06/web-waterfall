 ...
 */
/**
 * Test suite for `/api/identity/organization_units/ou-001` handlers.
 */
...
describe("/api/identity/organization_units/ou-001", () => {
  const IDENTITY_SERVICE_URL = "http://identity_service:5002";
  const UNIT_ID = "unit-id-001";
  let mockFetch: jest.Mock;
  let GETFn: (req: NextRequest, context: { params: Promise<{ unit_id: string }> }) => Promise<Response>;
  let PUTFn: (req: NextRequest, context: { params: Promise<{ unit_id: string }> }) => Promise<Response>;
  let PATCHFn: (req: NextRequest, context: { params: Promise<{ unit_id: string }> }) => Promise<Response>;
  let DELETEFn: (req: NextRequest, context: { params: Promise<{ unit_id: string }> }) => Promise<Response>;

  const buildReq = (body?: string): Partial<NextRequest> => {
    return {
      text: jest.fn().mockResolvedValue(body || ""),
      json: jest.fn().mockResolvedValue(body ? JSON.parse(body) : {}),
      url: `http://localhost:3000/api/identity/organization_units/ou-001`,
      headers: {
  ...
  describe("GET - MOCK_API=true", () => {
    ...
    it("returns mock OrganizationUnit by ID", async () => {
      const req = buildReq();
  ...
  describe("GET - MOCK_API=false", () => {
    ...
    it("proxies GET request", async () => {
        ...
        const mockJson = { id: "unit-id-001", name: "Test OrganizationUnit" };
        const mockRes = {
            status: 200,
            headers: {
  ...
  describe("PUT - MOCK_API=false", () => {
    ...
    it("proxies PUT request", async () => {
        ...
        const mockJson = { id: "unit-id-001", name: "Updated" };
        const mockRes = {
            status: 200,
            headers: {
                get: (key: string) => (key === "content-type" ? "application/json" : null),
  ...
  describe("PATCH - MOCK_API=false", () => {
    ...
    it("proxies PATCH request", async () => {
        ...
        const mockJson = { id: "unit-id-001", name: "Patched" };
        const mockRes = {
            status: 200,
            headers: {
                get: (key: string) => (key === "content-type" ? "application/json" : null),
