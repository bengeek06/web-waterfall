 ...
 */
/**
 * Test suite for `/api/identity/organization_units/[unit_id]/positions` handlers.
 */
...
describe("GET /api/identity/organization_units/[unit_id]/positions", () => {
  ...
  const buildReq = (): Partial<NextRequest> => {
    return {
      ...
      url: `http://localhost:3000/api/identity/organization_units/${UNIT_ID}/positions`,
      headers: {
  ...
  describe("MOCK_API=true", () => {
    ...
    it("returns mock positions list", async () => {
      const req = buildReq();
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies request and returns positions", async () => {
        const req = buildReq();
        const context = buildContext();
...
describe("POST /api/identity/organization_units/[unit_id]/positions", () => {
  const IDENTITY_SERVICE_URL = "http://identity_service:5002";
  const UNIT_ID = "ou-001";
  let mockFetch: jest.Mock;
  let POSTFn: (req: NextRequest, context: { params: Promise<{ unit_id: string }> }) => Promise<Response>;

  const buildReq = (): Partial<NextRequest> => {
    return {
      text: jest.fn().mockResolvedValue(JSON.stringify({ title: "New Position" })),
      json: jest.fn().mockResolvedValue({ title: "New Position" }),
      url: `http://localhost:3000/api/identity/organization_units/${UNIT_ID}/positions`,
      headers: {
  ...
  describe("MOCK_API=true", () => {
    ...
    it("returns mock created position", async () => {
      const req = buildReq();
      const context = buildContext();
  ...
  describe("MOCK_API=false", () => {
    ...
    it("proxies POST and creates position", async () => {
        const req = buildReq();
        const context = buildContext();
        
        const mockJson = { id: "pos-new", title: "New Position" };
        const mockRes = {
            status: 201,
            headers: {
                get: (key: string) => (key === "content-type" ? "application/json" : null),
