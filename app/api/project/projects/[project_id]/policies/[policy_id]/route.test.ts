...
describe('/api/project/projects/[project_id]/policies/[policy_id]', () => {
  ...
  const buildReq = (method = 'GET', body?: string) => {
    return {
      ...
      headers: new Headers(),
      url: `http://localhost:3000/api/project/projects/${projectId}/policies/${policyId}`,
      method,
  ...
  describe('GET', () => {
    it('should return policy details', async () => {
      ...
      const response = await GET(req, {
        params: Promise.resolve({ project_id: projectId, policy_id: policyId }),
      });

      expect(response.status).toBe(200);
      const data = await response.json();
      expect(data.id).toBe(policyId);
      expect(data.name).toBe('Edit Project');
    });
  });

  describe('PUT', () => {
    it('should update policy', async () => {
      const req = buildReq('PUT', JSON.stringify({ name: 'Edit Project Details' }));
