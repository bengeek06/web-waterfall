...
describe('GET /api/project/health', () => {
  ...
  const buildReq = () => {
    return {
      ...
      headers: new Headers(),
      url: 'http://localhost:3000/api/project/health',
  ...
  it('should return health status from upstream service', async () => {
    const mockResponse = {
      status: 'healthy',
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      `${PROJECT_SERVICE_URL}/health`,
      expect.any(Object)
    );

    const data = await response.json();
    expect(data.status).toBe('healthy');
    expect(data.service).toBe('project_service');
