 ...
 *
 * This module proxies requests to the guardian service for role-policy association operations.
 *
 * ## Supported HTTP methods:
 * - **DELETE**: Remove a policy from a specific role.
 *
...
/**
 * Handles DELETE requests to `/api/guardian/roles/[role_id]/policies/[policy_id]`.
 *
 * Proxies the request to the guardian service to remove a policy from a role.
 *
 * @param req - The incoming Next.js request object.
 * @param params - The route parameters containing role_id and policy_id.
...
export async function DELETE(
  ...
  logger.info(`DELETE request to /api/guardian/roles/${role_id}/policies/${policy_id}`);
  ...
  logger.debug(`Forwarding ${req.url} to ${GUARDIAN_SERVICE_URL}/roles/${role_id}/policies/${policy_id}`);
  ...
  const res = await serverSessionFetch(`${GUARDIAN_SERVICE_URL}/roles/${role_id}/policies/${policy_id}`, {
    method: "DELETE",
    headers,
  });
