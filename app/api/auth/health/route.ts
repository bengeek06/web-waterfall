 ...
 *
 * Proxies the request to the configured authentication service's `/health` endpoint,
...
/**
 * Handles GET requests to /api/auth/health endpoint.
 * @param req - The incoming Next.js request object.
 * @returns A NextResponse object containing the proxied response from the authentication service.
 */
export async function GET(req: NextRequest) {
  logger.info("GET request to /api/auth/health");

  if (process.env.MOCK_API === 'true') {
    logger.warn("Mocking authentication service response");
    const res = NextResponse.json({ status: "healthy", service: "auth_service", message: "Service is running" });
    return res;
  }
